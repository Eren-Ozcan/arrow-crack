...
- **Android audio focus**: music pauses when another app takes focus (a call,
  a podcast) and resumes only if it was playing before. In the WebView that
  arrives as the page being hidden, which is what `engine.ts` listens for —
  it suspends the context and cuts every sounding voice, and brings the loop
  back only when the loop was wanted. Effects still play at low volume with
  `mayDuck`.
